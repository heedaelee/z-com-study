...
export default function ActionButtons({ white, post }: Props) {
        ...
        if (queryKey[0] === "posts") {
          ...
          if (value && "pages" in value) {
            ...
            const repost = value.pages
              .flat()
              .find(
                (v) => v.Original?.postId === postId && v.User.id === session?.user?.email
              );
            if (obj) {
              // 존재는 하는지
              const pageIndex = value.pages.findIndex((page) => page.includes(obj));
  ...
  return (
    <div className={style.actionButtons}>
      ...
      {/* cx 스타일링 배열로 사용해도 됨 */}
      <div
        className={cx([style.heartButton, liked && style.liked, white && style.white])}
      >
